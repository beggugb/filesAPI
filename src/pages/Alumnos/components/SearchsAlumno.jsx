...
import SearchAlumno from "./SearchAlumno";

const SearchsAlumnos = () => {
    const dispatch = useDispatch()   
    const [modalView, setmodalView] = useState(false); 
    const { data  } = useSelector(state => state.alumnos) 
    ...
    const asignar = (item) => {    
        let est = modalView === true ? false : true;                     
        dispatch({type: 'VENTAS_CHANGE', props: 'alumnoId', value: item.id})
    ...
    return (                      
      <>
        <Row form>          
            <Col md={5}>        
             <FormGroup>
              <Label for="clients">Alumno</Label>
